...
class Home extends React.Component {
    ...
    getList(movie, i) {
        ...
        return (
            <li key={movie.imdbID} className="list-group-item">
                <Link to={topMovieUrl}>#{ (i+1) } - {movie.Title}</Link>
    ...
    render() {
        ...
        if(topMovie) {
            bannerMovies.push(topMovie);
        }
        let topSeries = this.getTop('series');
        if(topSeries) {
            bannerMovies.push(topSeries);
        }
                ...
                <section className="jumbotron text-center bg-dark">
                    ...
                    <div id="carouselExampleIndicators" className="carousel slide" data-ride="carousel">
                        ...
                        <div className="carousel-inner">
                            { bannerMovies.map(this.getBannerMovie)}
                        </div>
        ...
        let MovieList = this.state.movies.filter(movie => movie.Type === this.state.showType).map(this.getList);
